...
import A2Words from "../data/A2Words.json"; // JSON dosyasını buradan alıyoruz
...
export default function A2ContentScreen() {
  ...
  useEffect(() => {
    setLoadedWords(A2Words); // JSON'dan kelimeleri yükle
    setRandomWord(A2Words[Math.floor(Math.random() * A2Words.length)]); // Rastgele kelime seç
  }, []);
  ...
  const handleIknowWord = () => {
    ...
    setRandomWord(A2Words[Math.floor(Math.random() * A2Words.length)]);
  };
