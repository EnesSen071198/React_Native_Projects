...
import C2Words from "../data/C2Words.json"; // JSON dosyasını buradan alıyoruz
...
export default function C2ContentScreen() {
  ...
  useEffect(() => {
    setLoadedWords(C2Words); // JSON'dan kelimeleri yükle
    setRandomWord(C2Words[Math.floor(Math.random() * C2Words.length)]); // Rastgele kelime seç
  }, []);
  ...
  const handleIknowWord = () => {
    ...
    setRandomWord(C2Words[Math.floor(Math.random() * C2Words.length)]);
  };
