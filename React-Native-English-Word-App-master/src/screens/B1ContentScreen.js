...
import B1Words from "../data/B1Words.json"; // JSON dosyasını buradan alıyoruz
...
export default function B1ContentScreen() {
  ...
  useEffect(() => {
    setLoadedWords(B1Words); // JSON'dan kelimeleri yükle
    setRandomWord(B1Words[Math.floor(Math.random() * B1Words.length)]); // Rastgele kelime seç
  }, []);
  ...
  const handleIknowWord = () => {
    ...
    setRandomWord(B1Words[Math.floor(Math.random() * B1Words.length)]);
  };
