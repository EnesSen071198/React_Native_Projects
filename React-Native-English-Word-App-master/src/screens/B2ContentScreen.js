...
import B2Words from "../data/B2Words.json"; // JSON dosyasını buradan alıyoruz
...
export default function BContentScreen() {
  ...
  useEffect(() => {
    setLoadedWords(B2Words); // JSON'dan kelimeleri yükle
    setRandomWord(B2Words[Math.floor(Math.random() * B2Words.length)]); // Rastgele kelime seç
  }, []);
  ...
  const handleIknowWord = () => {
    ...
    setRandomWord(B2Words[Math.floor(Math.random() * B2Words.length)]);
  };
